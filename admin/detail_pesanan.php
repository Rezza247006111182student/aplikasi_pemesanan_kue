...
?>
        ...
        if (isset($_GET['id'])) {
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<div><strong>ID Pesanan:</strong> " . $pesanan['id'] . "</div>";
                echo "<div><strong>User:</strong> " . $pesanan['username'] . "</div>";
                ...
                echo "<div><strong>Status:</strong> <span class='px-2 inline-flex text-xs leading-5 font-semibold rounded-full " . ($pesanan['status'] == 'pending' ? 'bg-yellow-100 text-yellow-800' : ($pesanan['status'] == 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800')) . "'>" . ucfirst($pesanan['status']) . "</span></div>";
                echo "</div>";
                echo "</div>";
